...
DB="$LEADBOT_DIR/data/apex_ai.db"
PYTHON="$LEADBOT_DIR/venv/bin/python"
LOG="$LEADBOT_DIR/logs/watchdog.log"
INDEXER_LOG="$LEADBOT_DIR/logs/indexer.log"
...
if [ "$NEEDS_RESTART" -eq 1 ]; then
    ...
    if [ ! -f "$LEADBOT_DIR/src/parser_apex_ai.py" ]; then
        log "❌ Нет файла $LEADBOT_DIR/src/parser_apex_ai.py"
        exit 1
    fi
    if [ ! -x "$PYTHON" ]; then
