"""
live_push.py — Concierge live-monitor for paid clients (v2 with AI-qualifier hook).

Что изменилось vs v1:
1. Чёрный список чатов (blacklist.py) — отсекаем «работа/вакансии» ДО FTS5.
2. Эксклюзивность: один corpus_id попадает максимум ОДНОМУ клиенту.
   Если уже есть в pending/ai_pending/sent у любого — другому не пойдёт.
3. INSERT идёт со status='ai_pending' — лид ждёт прогона через AI-qualifier.
   В /review админу попадает только то, что AI пропустил (status='pending').

Совместимо со старой БД: использует те же таблицы paid_clients, pending_review,
delivered_leads, messages_corpus + messages_fts.
"""
...
# Локальный импорт (deploy_ai.sh кладёт blacklist.py рядом)
try:
    from blacklist import should_skip_chat
except ImportError:  # на случай если запуск из другой папки
    import sys
...
POLL_INTERVAL = 30                 # сек между опросами
NOTIFY_COOLDOWN = 30 * 60          # сек между уведомлениями админа
MAX_MESSAGE_AGE_DAYS = 7
MAX_MATCHES_PER_CLIENT_PER_TICK = 200

# В какой статус класть новых кандидатов. AI-qualifier потом промоутит в 'pending'.
...
# ==========================================
# 🗄️  ИНИЦИАЛИЗАЦИЯ ТАБЛИЦ + МИГРАЦИЯ
# ==========================================
async def _ensure_columns(db: aiosqlite.Connection) -> None:
    """Idempotent ALTER TABLE для AI-полей."""
    cols = [
        ("ai_score", "INTEGER"),
        ("ai_pain", "TEXT"),
        ("ai_fit_service", "TEXT"),
...
async def init_tables(db_path: str = APEX_DB) -> None:
    async with aiosqlite.connect(db_path) as db:
        ...
        """)
        await db.execute(
            "CREATE INDEX IF NOT EXISTS idx_paid_status ON paid_clients(status)"
        )
        ...
        await db.execute(
            "CREATE INDEX IF NOT EXISTS idx_pending_client ON pending_review(client_user_id, status)"
        )
        await db.execute(
            "CREATE INDEX IF NOT EXISTS idx_pending_ai_status ON pending_review(status, id)"
        )
        await db.execute(
            "CREATE INDEX IF NOT EXISTS idx_pending_corpus_status ON pending_review(corpus_id, status)"
        )
        ...
        """)
        await db.execute(
            "CREATE INDEX IF NOT EXISTS idx_delivered_client ON delivered_leads(client_user_id)"
        )
        # Индекс для эксклюзивности (быстрая проверка corpus_id уже доставлен)
        await db.execute(
            "CREATE INDEX IF NOT EXISTS idx_delivered_corpus ON delivered_leads(corpus_id)"
        )

        await db.commit()
    logger.info("📂 Таблицы готовы (v2 с AI-полями)")
...
# ==========================================
# 🔍 ПОИСК МАТЧЕЙ (с эксклюзивностью)
# ==========================================
async def scan_for_client(client: dict, db_path: str = APEX_DB) -> list[tuple[int, str | None]]:
    """
    Возвращает [(corpus_id, chat_key), ...] которые:
    - матчатся по FTS под нишу клиента
    - НЕ старее 7 дней
    - НЕ доставлены ему ранее
    - НЕ существуют в pending_review со status IN (ai_pending, pending, sent)
      у ЛЮБОГО клиента (эксклюзивность!)

    Чёрный список чатов фильтруется уже после возврата в queue_candidates,
    тут оставляем chat_key чтобы это сделать.
    """
    keywords = parse_keywords(client["keywords"])
    if not keywords:
        return []
...
async def main_loop() -> None:
    await init_tables()
    logger.info(
        "📡 live_push v2 запущен. POLL=%ds, AGE=%dd, ADMIN=%s, status='%s'",
        POLL_INTERVAL,
            ...
            if clients:
                ...
                if (pending_since_notify > 0 or blacklisted_since_notify > 5) and age >= NOTIFY_COOLDOWN:
                    msg = (
                        ...
                        f"⏭ {blacklisted_since_notify} blacklist-чатов отсеяно"
                    )
