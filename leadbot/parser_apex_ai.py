...
SESSION_NAME = 'apex_discoverer_v.session'
ADMIN_ID = 8128303065
ALERT_BOT_TOKEN = os.environ.get('BOT_TOKEN') or os.environ.get('ALERT_BOT_TOKEN', '')

# БД и бюджет — рядом со скриптом (чтобы парсер, бот, query_engine видели одно).
APEX_DB = os.environ.get("APEX_DB") or str(Path(__file__).resolve().parent / 'apex_ai.db')
DAILY_BUDGET_FILE = str(Path(__file__).resolve().parent / 'parser_budget.json')
